...
# Training parameters
start_epoch = 0
epochs = 16  # number of epochs to train for (if early stopping is not triggered)
...
def main():
    ...
    # Read word map
    word_map_file = os.path.join('./datasets', 'WORDMAP_' + 'COCO_Knowledge5k' + '.json')
    with open(word_map_file, 'r') as j:
        word_map = json.load(j)

#     checkpoint = './checkpoint_24_coco_pretrain.pth.tar'
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # Save checkpoint
        if epoch % 5 == 0:
            save_checkpoint('show_att_tell_coco_{}'.format(epoch), epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer,
...
def train(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch):
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        one_hot_knowledge.scatter_(1, [[0]], 1)
        one_hot_opinion.scatter_(1, [[0]], 1)
        one_hot_gender.scatter_(1, [[0]], 1)
...
def train_coco(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch):
    """
    Performs one epoch's training.
    :param train_loader: DataLoader for training data
    :param encoder: encoder model
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        one_hot_knowledge = torch.FloatTensor(caps.shape[0], 4)
        one_hot_opinion = torch.FloatTensor(caps.shape[0], 6)
        one_hot_gender = torch.FloatTensor(caps.shape[0], 3)
        ...
        index = torch.LongTensor([0]*caps.shape[0]).reshape(caps.shape[0],1)

        one_hot_knowledge.scatter_(1, index, 1)
        one_hot_opinion.scatter_(1, index, 1)
        one_hot_gender.scatter_(1, index, 1)
...
def validate_coco(val_loader, encoder, decoder, criterion):
    """
    Performs one epoch's validation.
    :param val_loader: DataLoader for validation data.
    :param encoder: encoder model
    ...
    with torch.no_grad():
        ...
        for i, (imgs, caps, caplens, allcaps) in enumerate(val_loader):
            ...
            # print(scores.size(), decode_lengths.size())
